%TAKES RANDOM SAMPLES OF SAMPLE OBSERVATIONS FROM _some_ COLUMNS-TREEBAGGER
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
clear;
clc;
...
numChosenSubsets = 12;%millions of observations->try to keep to 15 or under
...
for i = 1:numTimesToSample %sample numChosenSubsets subsets numTimesToSample times
    ...
    if (rand() <= numChosenSubsets/41)%add last sub column to the population 20/41 of the time(results in numChosenSubsets+1 subsets)
        disp('@41st column');
...
hdf5write('matlabPredictions2.h5', '/data', prediction_vector);

disp('writing predictions to csv file...');
% write csv file
system('java -jar C:/Users/meehanb/Documents/NetBeansProjects/WriteCSVpredictions/dist/WriteCSVpredictions.jar matlabPredictions2.h5 clickDataPredictions2.csv');
